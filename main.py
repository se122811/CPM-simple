    ...
    def train(self):
        ...
        for epoch in tqdm.tqdm(range(self.epochs + 1)):
            ...
            for batch_idx, samples in enumerate(self.dataloader):
                ...
                # loss
                loss = self.loss(heatmapsPoseNet,gt_heatmap)
                # optim
            ...
            heatmapsPoseNet = self.poseNet(x_train.cuda()).cpu().detach().numpy()
...
def plot_hand(img,joints):
    ...
    for row_idx, row in enumerate(joints):
        if joint_idxs:
            plt.annotate(str(row_idx), (row[1], row[0]))
...
def _draw2dseg(ax, annot, idx1, idx2, c='r', alpha=1, linewidth=1):
    ax.plot([annot[idx1, 1], annot[idx2, 1]], [annot[idx1, 0], annot[idx2, 0]],
            c=c,
            alpha=alpha,
            linewidth=linewidth)
